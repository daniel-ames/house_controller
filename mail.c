...
event_cb (smtp_session_t session, int event_no, void *arg,...)
{
  ...
  case SMTP_EV_MAILSTATUS:
    ...
    printf("MAILSTATUS - status code: %d text: %s", status->code, status->text == NULL ? "null" : status->text); // omit newline because status code always has one
    break;
  case SMTP_EV_RCPTSTATUS:
    ...
    printf("SMTP_EV_RCPTSTATUS - status code: %d text: %s", status->code, status->text == NULL ? "null" : status->text); // omit newline because status code always has one
    break;
  ...
  default:
    // don't hassle me with details
    // if (event_no == SMTP_EV_MESSAGEDATA || event_no == SMTP_EV_DISCONNECT || event_no == SMTP_EV_STARTTLS_OK)
    //   break;
    // what's this?
